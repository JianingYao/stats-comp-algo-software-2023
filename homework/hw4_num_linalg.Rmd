...
**My answer: **
Theoretically, the overall costs of solving a single linear system involving symmetric positive definite matrix via LU decomposition, Cholesky decomposition, and QR decomposition are $O(\frac{2}{3}p^3)$, $O(\frac{1}{3}p^3)$, and $O(\frac{4}{3}p^3)$. However, the relative speeds do not agree with the theoretical costs, where LU decomposition using solve() takes 134ms (median), Cholesky using chol() and backsolve() takes 173ms (median), and QR using qr.solve() takes 496ms (median). solve() is the fastest because it uses an optimized algorithm to solve linear systems with LAPACK library, which is faster than using the built-in chol() and backsolve() that take time to allocate space to store and access the upper triangular matrix in our user-defined function. 
...
**My answer:**
When the condition number increases, the coordinate-wise relative error increases across three algorithms and metrics. That's because the problem is more ill-conditioned for large k and the algorithms are more likely to suffer from numerical instability. When the condition number is low or the problem is well-conditioned, LU decomposition using solve() and Cholesky decomposition yield similar low relative error, compared with QR decomposition. When the condition number is high, Cholesky decomposition has the highest numerical stability for a system with positive definite matrices. QR decomposition has the highest relative error for all three condition numbers. The patterns remain similar across the five metrics. Please refer to the saved png files in the folder.
