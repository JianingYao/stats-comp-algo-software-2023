...
**Your answer:** The size of the git_experiment is 25M, whereas the total size of all individuals files inside it is 75K. This is because every time I ran and committed to the github iteratively, log files would be saved in the hidden folder .git/. Github discourages users from committing large binary files as these files will take up large space in the repository, decreasing the efficiency of cloning and pulling. Large binary files also cost unnecessary effort to maintain the GitHub platform. 
...
https://github.com/JianingYao/hiperglm.git 
