...
print(paste0("Sample second moment is: ", sample_sec_mom))
print(paste0("Sample variance is: ", sample_var_with_question_mark))
```
...
```{r}
print(.Machine$double.digits)
```


## Things to commit/submit

**MY ANSWER**: When the seed is 140778, the sample mean is 9999999999.97963. However, the sample variance is -147456, which does not make sense since variance should be positive. When the seed is 11234, the sample mean is 10000000000.0682 and the sample variance is 32768. However, the sample variance should be 1 theoretically or near to 1 empirically when using Monte Carlo simulation. The above errors occur because when we calculate sum of squares by adding $\theta^2$ to the sum or calculate squared sample mean, the results (at least $10^{20}$ or $2^{66}$) exceed the maximum number of decimal digits (53) that can be represented precisely in a double precision floating-point number. So any decimal number with more than 16 digits will be rounded or truncated when calculated in R. Such errors can accumulate and result in significantly inaccurate estimate of the true sample variance during each iteration of the Monte Carlo simulation. 
...
**MY ANSWER**: the expected slope of the plotted line should be -2 because the difference between analytically derivative and numerical derivative ($O((\Delta x)^2)$) should decrease when $\Delta x$ decreases. Following the equation by taking log on both side, we can get $\log_2 |f'(x) - \frac{f(x+\Delta x) - f(x-\Delta x)}{2\Delta x}| \leq -2\log_2 \Delta x + const$. So the expected slope is -2 on a log-log scale. 

The relative error plot shows negative slope (roughly to be -2) when the step size is larger than $2^{-16}$, but the slope becomes positive afterwards, meaning the relative error increases as the step size is smaller. The slope in the latter part of the plot is roughly 1. The relative error plot does not agree with the mathematical theory because of the limited numerical precision of the computer and rounding error. Considering a machine with 10 decimal digits of accuracy, the machine will only calculate precisely when the step size is larger than $2^{-16}$ ($\approx10^{-5}$) and the error will accumulate when we divided by this rounded $\Delta x$, resulting in increase of relative error. If we continue to decrease the step size below $2^{-52}$, the relative error will remain at 1. (The graph is generated in the folder named as relative_error.jpg)
...
To derive the analytical expression for the gradient of $f(\boldsymbol{x}) = - \boldsymbol{x}^\intercal \boldsymbol{\Phi} \boldsymbol{x} / 2$, we can apply the chain rule and get $\Delta f(\boldsymbol{x}) = -\boldsymbol{\Phi} \boldsymbol{x}$. 
...
**Answers are embedded in the questions.** 
...
https://github.com/JianingYao/hiperglm/pull/1 
